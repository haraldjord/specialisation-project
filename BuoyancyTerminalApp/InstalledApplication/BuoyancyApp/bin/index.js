...
const noble = require('noble-mac');                       /**<  To Handle BLE*/
...
const buoyancyAppDir = path.join('.', '..', 'BuoyancyLog');
                        ...
                        if (char.properties.includes('notify')) { /**< If notification is enabled, read character fron Nordic UART Service */
                            ...
                            uartReadChar.on('data', function(data) {  /**< Incoming data from BLE*/
                                ...
                                if(Msg[0] === '/'){ /**< First character of received message is '/' indicate the start of a new file transmission */
                                  ...
                                  byteCnt = 0;
                                    ...
                                    if (fs.existsSync(filepath)) {
                                        writestream = fs.createWriteStream(path.join(missionLogDir,`new.${filename[1]}`));             /**< In the case of dublicate filename add 'new.' to the filename to indicate a new transfered file with the same name*/
                                    }else{
                                        writestream = fs.createWriteStream(filepath);
                                    }
                                    transfer = true;

                                }else if(Msg === "##"){ /**< "##" indicate the end of current file transmission */
                                console.log('');
                                transfer = false;
                                }
                                ...
                                if(transfer == true && Msg[0] != '/'){
                                    ...
                                    writestream.write(data, 'utf-8', err)  /**< Write incoming BLE data to file*/
                                    byteCnt += data.length;
